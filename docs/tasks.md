...
- [x] **T3.3 — Añadir en una capa.** Botón por capa que crea nota asignada. *Hecho cuando:* aparece en esa capa.

## Etapa 4 · Tablero

- [x] **T4.1 — Render del tablero.** Columnas por estado con sus notas. *Hecho cuando:* se agrupan por estado.
- [x] **T4.2 — Mover entre columnas.** Arrastrar cambia `status` y persiste. *Hecho cuando:* sobrevive a recarga.
...
- [x] **T5.3 — Backlinks.** Enlaces salientes y backlinks calculados con `core/`. *Hecho cuando:* reflejan los enlaces reales.

## Etapa 6 · Lienzo y grafo

- [x] **T6.1 — Lienzo (React Flow).** Nodos posicionables, pan y zoom. *Hecho cuando:* la posición persiste.
- [x] **T6.2 — Conexiones.** Crear y borrar; persistir en `edges`. *Hecho cuando:* sobreviven a recarga.
- [x] **T6.3 — Grafo.** Nodos + aristas (conexiones y wikilinks); clic lleva al documento. *Hecho cuando:* se ven ambos tipos y navega.
